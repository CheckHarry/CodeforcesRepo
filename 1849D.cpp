...
void dfs(int pos, const vector<int>& v, vector<bool>& mark, int &ans, bool a) {
    if (mark[pos] == 0) {
        mark[pos] = 1;
        if (!a)
            ans ++;
    ...
    for (int i = 0; i < v[pos]; i ++) {
        if (pos - 1 >= 0 && mark[pos - 1] == 0) {
            dfs(pos - 1, v, mark, ans, true);
            continue;
        }

        if (pos + 1 < v.size() && mark[pos + 1] == 0) {
            dfs(pos + 1, v, mark, ans, true);
            continue;
        }
    }
...
int solve(const vector<int>& v)  {
    ...
    for (;i < v.size(); i ++) {
        if (v[i] == 0 && left != -1) {
            segs.push_back({left,i - 1,has_two});
            left = -1;
            has_two = false;
        } else {
            left = i;
